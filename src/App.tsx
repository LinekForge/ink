...
import { useExternalFilePoll } from './hooks/useExternalFilePoll'
...
function App() {
  ...
  useExternalFilePoll(activeTab?.path, activeTab?.id, activeTab?.savedRevision)
  ...
  // ─── 启动：只处理 argv 文件，不自动 restore session ───────
  // 设计：冷启动干干净净——有 argv 就打开 argv；没 argv 就 Welcome，
  // Welcome 上面有"恢复上次会话"按钮，让用户主动选择。
  useEffect(() => {
    ;(async () => {
